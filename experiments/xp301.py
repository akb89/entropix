...
if __name__ == '__main__':
    ...
    for tuple1, tuple2 in itertools.combinations(models, 2):
        ...
        for idx in tqdm(range(model1.shape[1])):
            ...
            xcorr = abs(metrix.pearson_correlation(model1[:, idx], model2[:, idx]))
            xcorrx.append(xcorr)
        avgs = com_xp.binize(np.array(xcorrx), BIN_SIZE).mean(axis=1)
        with open('{}-{}-pearson-rmse.dat'.format(name1, name2), 'w', encoding='utf-8') as outs:
            for avg, rmse in zip(avgs, xcorrx):
