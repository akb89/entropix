...
def generated_biased_dims(svd_dims, bias_dims, bins_size):
    ...
    for idx in svd_dims:
        ...
        if block_idx < len(blocks):
            probs.append(block_probs[block_idx] / bins_size)
        else:
            probs.append(0.)
    assert np.isclose(1, sum(x for x in probs))
    return sorted(np.random.choice(svd_dims, size=len(bias_dims),
...
def get_dataset_coverage(dataset, vocab):
    """Get number of items covered in dataset given vocab."""
...
def load_model_and_vocab(model_filepath, model_type, vocab_filepath=None,
    ...
    if randomize and dims_filepath:
        raise Exception('Cannot specify both --dims and --randomize')
    ...
    if model_type == 'gensim':
        ...
        if model_type == 'scipy':
            ...
            if model_type == 'numpy':
                ...
                if dims_filepath and dims and not shuffle:
                ...
                if shuffle:
                    ...
                    if not dims_filepath:
                        logger.warning(
                            ...
                            'specific bias'.format(ndim, model.shape[1]))
                        ...
                        logger.warning(
                            'Shuffle mode ON: will randomly select {} '
                            'dimensions out of {} in the SVD matrix following '
                            'bias specified in input dims_filepath'
                            .format(len(dims), ndim))
                        svd_dims = list(range(model.shape[1]))
                        logger.info('Bias dims = {}'.format(dims))
                        sampled_dims = generated_biased_dims(svd_dims, dims,
                                                             bins_size)
                        logger.info('Sampled dims from bias = {}'
                                    .format(sampled_dims))
                        model = model[:, sampled_dims]
            else:
                if model_type == 'ica':
