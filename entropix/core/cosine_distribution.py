...
def _load_space(space_filepath):
    ...
    if not os.path.exists(os.path.join(os.path.dirname(space_filepath), map_filepath)):
...
def cosine_distribution(output_dirpath, space_filepath, threads_number, bin_size,
                        vocabulary_filepath=''):
    ...
    if vocabulary:
        idx_to_word_dic = {k: w for k, w in idx_to_word_dic.items()
                           if w in vocabulary}
        logger.info('{} out of {} vocabulary items found.'.format(len(idx_to_word_dic),
                                                                  len(vocabulary)))
    number_of_bins = 1/bin_size
    freqdist = [0]*int(number_of_bins)
    cosinepairs_filepath = os.path.join(output_dirpath,
                                        '{}.cos.gz'.format(os.path.basename(space_filepath)))
    with gzip.open(cosinepairs_filepath, 'wt', encoding='utf-8') as output_stream:
        with multiprocessing.Pool(threads_number) as pool:
            ...
            for partial_cosine_dic, idx in tqdm(pool.imap_unordered(process,
                ...
                for index_pair, cosine in partial_cosine_dic.items():
                    ...
                    print('{}\t{}\t{}'.format(idx_to_word_dic[idx],
                                              idx_to_word_dic[idx2], cosine),
                           file=output_stream)

    distribution_filepath = os.path.join(output_dirpath, 'cosine_distribution.txt')
    with open(distribution_filepath, 'w', encoding='utf-8') as output_stream:
        print('range\tN', file=output_stream)
