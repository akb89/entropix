...
#    euclidean = mutils.similarity(submatrix_left, submatrix_right, 'euclidean')
    ...
    for left_w, right_w in pairs_dataset:
        ...
        x_reduced = x_row[0,nonzero_indices].todense()
        y_reduced = y_row[0,nonzero_indices].todense()
#        print(x_reduced)
#        print(y_reduced)
        row_wise_euclidean.append(np.linalg.norm(x_reduced - y_reduced))

#        print(row_wise_euclidean)
#        input()
        nonzero_values = [x_inters_y[0,i] for i in nonzero_indices]
        ...
        sorted_value_idx_pair = sorted(zip(nonzero_values,nonzero_indices), reverse = True)
        sorted_values = [x[0] for x in sorted_value_idx_pair]

        for p in range(10, 100, 10):
            percentiles[p].append(np.percentile(sorted_values, p))
        for p in range(50):
            positions[p].append(sorted_values[min(len(sorted_values)-1, p)])
...
#    euclidean_distances = [euclides[x][y] for x, y in pairs_dataset]
    ...
    for p in range(10, 100, 10):
        print ('value at percentile', p, stats.spearmanr(percentiles[p], dataset_sim)[0])

    for p in range(50):
        print ('value at position', p, stats.spearmanr(positions[p], dataset_sim)[0])
